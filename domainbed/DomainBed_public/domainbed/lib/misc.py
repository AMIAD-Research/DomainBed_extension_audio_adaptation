...
def stratified_split_dataset(dataset, n, seed=0):
    assert n <= len(dataset)

    rng = np.random.RandomState(seed)
    total = len(dataset)

    labels = [dataset[i][1] for i in range(total)]
    ...
    if n < num_classes:
        raise ValueError("Cannot have at least 1 sample per class if n < num_classes")
    ...
    for y, idxs in class_indices.items():
        ...
        q = k * n / total
        quotas[y] = q
        floors[y] = int(np.floor(q))
        remainders[y] = q - floors[y]

    # --- Step 2: enforce at least 1 per class ---
    for y in classes:
        if floors[y] == 0:
            floors[y] = 1

    # --- Step 3: fix total count ---
    current_total = sum(floors.values())

    if current_total > n:
        # remove from largest allocations first
        surplus = current_total - n
        sorted_classes = sorted(classes, key=lambda y: floors[y], reverse=True)

        for y in sorted_classes:
            while floors[y] > 1 and surplus > 0:
                floors[y] -= 1
                surplus -= 1
                if surplus == 0:
                    break
        ...
        # add based on largest remainder
        deficit = n - current_total
        sorted_classes = sorted(classes, key=lambda y: remainders[y], reverse=True)
    ...
    for y, idxs in class_indices.items():
        ...
        n1 = min(floors[y], len(idxs))  # safety
        keys_1.extend(idxs[:n1])
        keys_2.extend(idxs[n1:])
...
def accuracy(network, loader, weights, device):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            if p.size(1) == 1:
                batch_correct += (p.gt(0).eq(y).float() * batch_weights.view(-1, 1)).sum()
            else:
                batch_correct += (p.argmax(1).eq(y).float() * batch_weights).sum()
            
            batch_total += batch_weights.sum()
            
            batch_acc = batch_correct / batch_total
            batch_accuracies.append(batch_acc.item())

            correct += batch_correct.item()
            total += batch_total.item()

    network.train()

    global_acc = correct / total
    variance = torch.tensor(batch_accuracies).var(unbiased=False).item()
    
    return global_acc, variance

from collections import defaultdict
def accuracy_per_class(network, loader, weights, device):
