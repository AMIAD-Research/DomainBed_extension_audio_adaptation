...
class Algorithm(torch.nn.Module):
    ...
    def update(self, minibatches, target=None):
        """
        Perform one update step, given a list of (x, y) tuples for all
        environments.

        Admits an optional list of target minibatches from the test domains,
...
class ERM(Algorithm):
    ...
    def update(self, minibatches, target=None):
        ...
        source_all_x = torch.cat([x for x, y in minibatches])
        source_all_y = torch.cat([y for x, y in minibatches])
        source_loss = F.cross_entropy(self.predict(source_all_x), source_all_y)

        # Adaptation loss calculation
        if target and isinstance(target[0], tuple): # supervised_domain_adaptation
            target_x = torch.cat([x for x, y in target])
            target_y = torch.cat([y for x, y in target])

            target_loss = F.cross_entropy(self.predict(target_x), target_y)

        elif target and isinstance(target[0], torch.Tensor): # unsupervised_domain_adaptation
            # unkown method 
            raise ValueError(f"unsupervised_domain_adaptation not yet implemented in ERM..")

        loss = source_loss + self.hparams["erm_adapt_coeff"] * target_loss # target_loss = 0 in DG
    ...
    def featurize(self, minibatches):
        with torch.no_grad():
            all_x = torch.cat([x for x, y in minibatches])
            all_y = torch.cat([y for x, y in minibatches])
        return self.featurizer(all_x), all_y
...
class AbstractDANN(Algorithm):
    ...
    def update(self, minibatches, target=None):
        ...
        source_all_x = torch.cat([x for x, y in minibatches])
        source_all_y = torch.cat([y for x, y in minibatches])
        source_all_z = self.featurizer(source_all_x)

        if target is not None: 
            # ===== Supervised Domain Adaptation =====
            if isinstance(target[0], tuple):
                target_all_x = torch.cat([x for x, y in target])
                target_all_y = torch.cat([y for x, y in target])
                target_all_z = self.featurizer(target_all_x)
            # ===== Unsupervised Domain Adaptation =====
            elif isinstance(target[0], torch.Tensor):
                target_all_x = torch.cat([x for x in target])
                target_all_z = self.featurizer(target_all_x)


        if self.conditional:
            source_disc_input = source_all_z + self.class_embeddings(source_all_z)
            if target is not None: # If adaptation
                target_disc_input = target_all_z + self.class_embeddings(target_all_z)
        else:
            source_disc_input = source_all_z
            if target is not None: # If adaptation
                target_disc_input = target_all_z

        ## FR
        source_disc_input = source_disc_input.detach().requires_grad_(True)
        if target is not None:
            target_disc_input = target_disc_input.detach().requires_grad_(True)
        ##

        source_disc_out = self.discriminator(source_disc_input)
        source_disc_labels = torch.cat([
            torch.full((x.shape[0], ), i, dtype=torch.int64, device=device)
            for i, (x, y) in enumerate(minibatches)
        ])

        if target is not None: 
            target_disc_out = self.discriminator(target_disc_input)
            if isinstance(target[0], tuple): # ===== Supervised Domain Adaptation =====
                target_disc_labels = torch.cat([
                    torch.full((x.shape[0],), i, dtype=torch.int64, device=device)
                    for i, (x, y) in enumerate(target)
                ])
            elif isinstance(target[0], torch.Tensor):# ===== Unsupervised Domain Adaptation ===== 
                target_disc_labels = torch.cat([
                    torch.full((x.shape[0],), i, dtype=torch.int64, device=device)
        ...
        if self.class_balance:
            y_counts = F.one_hot(source_all_y).sum(dim=0)
            source_weights = 1. / (y_counts[source_all_y] * y_counts.shape[0]).float()
            source_disc_loss = F.cross_entropy(source_disc_out, source_disc_labels, reduction='none')
            source_disc_loss = (source_weights * source_disc_loss).sum()
            if target is not None: 
                target_y_counts = F.one_hot(target_all_y).sum(dim=0)
                target_weights = 1. / (target_y_counts[target_all_y] * target_y_counts.shape[0]).float()
                target_disc_loss = F.cross_entropy(target_disc_out, target_disc_labels, reduction='none')
                target_disc_loss = (target_weights * target_disc_loss).sum()
            
        else:
            source_disc_loss = F.cross_entropy(source_disc_out, source_disc_labels)
            if target is not None: 
                target_disc_loss = F.cross_entropy(target_disc_out, target_disc_labels)

        source_input_grad = autograd.grad(
            F.cross_entropy(source_disc_out, source_disc_labels, reduction='sum'),
            [source_disc_input], create_graph=True)[0]
        source_grad_penalty = (source_input_grad**2).sum(dim=1).mean(dim=0)
        source_disc_loss += self.hparams['grad_penalty'] * source_grad_penalty
        disc_loss = source_disc_loss

        if target is not None: 
            target_input_grad = autograd.grad(
            F.cross_entropy(target_disc_out, target_disc_labels, reduction='sum'),
            [target_disc_input], create_graph=True)[0]
            target_grad_penalty = (target_input_grad**2).sum(dim=1).mean(dim=0)
            target_disc_loss += self.hparams['grad_penalty'] * target_grad_penalty
            disc_loss += self.hparams['dann_adapt_coeff_disc'] * target_disc_loss
        ...
        if (self.update_count.item() % (1+d_steps_per_g) < d_steps_per_g):
            ...
            source_all_preds = self.classifier(source_all_z)
            source_classifier_loss = F.cross_entropy(source_all_preds, source_all_y)
            source_gen_loss = (source_classifier_loss +
                        (self.hparams['lambda'] * -source_disc_loss))
            gen_loss = source_gen_loss

            if target is not None: 
                # ===== Supervised Domain Adaptation =====
                if isinstance(target[0], tuple):
                    target_all_preds = self.classifier(target_all_z)
                    target_classifier_loss = F.cross_entropy(target_all_preds, target_all_y)
                    target_gen_loss = (target_classifier_loss +
                                (self.hparams['lambda'] * -target_disc_loss))
                    gen_loss += self.hparams['dann_adapt_coeff_gen'] * target_gen_loss
...
class RDM(ERM):
    ...
    def update(self, minibatches, target=None, held_out_minibatches=None):
...
class AbstractMMD(ERM):
    ...
    def update(self, minibatches, target=None):
        ...
        source_obj = 0
        source_pen = 0
        source_nmb = len(minibatches)

        source_features = [self.featurizer(xi) for xi, _ in minibatches]
        source_classifs = [self.classifier(fi) for fi in source_features]
        source_labels = [yi for _, yi in minibatches]

        for i in range(source_nmb):
            source_obj += F.cross_entropy(source_classifs[i], source_labels[i])
            for j in range(i + 1, source_nmb):
                source_pen += self.mmd(source_features[i], source_features[j])

        source_obj /= source_nmb
        if source_nmb > 1:
            source_pen /= (source_nmb * (source_nmb - 1) / 2)

        total_objective = source_obj
        total_penalty = source_pen

        results = {
            "total_obj": source_obj.item(),
            "total_penalty": source_pen.item() if torch.is_tensor(source_pen) else source_pen,
        }
        ...
        if target is not None:
            ...
            # ===== Supervised Domain Adaptation =====
            if isinstance(target[0], tuple):
                target_nmb = len(target)

                target_features = [self.featurizer(xi) for xi, _ in target]
                target_classifs = [self.classifier(fi) for fi in target_features]
                target_labels = [yi for _, yi in target]

                for i in range(target_nmb):
                    target_obj += F.cross_entropy(target_classifs[i], target_labels[i])
                    for j in range(source_nmb):
                        target_pen += self.mmd(target_features[i], source_features[j])
                    if target_nmb > 1: 
                        for k in range(i+1, target_nmb):
                            target_pen += self.mmd(target_features[i], target_features[k])

                target_obj /= target_nmb
                if target_nmb > 0 and source_nmb > 0:
                    target_pen /= (target_nmb * (2*source_nmb + target_nmb - 1) /2)
            ...
            # ===== Unsupervised Domain Adaptation =====
                ...
                target_features = [self.featurizer(xi) for xi in target]
                target_classifs = [self.classifier(fi) for fi in target_features]
            ...
            total_objective = source_obj + self.hparams['mmd_adapt_coeff_obj'] * target_obj # target_obj=0 in UDA 

            ## Remove inter-source domain regularization: small test
            # print("NO INTER-SOURCE DOMAINS REGULARIZATION")
            # source_pen = 0
            ## 

            total_penalty = source_pen + self.hparams['mmd_adapt_coeff_pen'] * target_pen
            results.update({
                "target_obj": target_obj.item() if torch.is_tensor(target_obj) else target_obj,
                "target_penalty": target_pen.item() if torch.is_tensor(target_pen) else target_pen,
                "total_obj": total_objective.item() if torch.is_tensor(total_objective) else total_objective, 
                "total_penalty": total_penalty.item() if torch.is_tensor(total_penalty) else total_penalty,
            })


        total_loss = total_objective + (self.hparams['mmd_gamma']*total_penalty)
        results.update({
...
class Transfer(Algorithm):
    ...
    def update_second(self, minibatches, target=None):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
...
class FSP(Algorithm): # FewShotPrototype
    def __init__(self, input_shape, num_classes, model_featurizer, hparams):
        super().__init__(input_shape, num_classes, model_featurizer, hparams)

        self.num_classes = num_classes

        self.featurizer = model_featurizer
    ...
    def compute_prototypes(self, loader):
        ...
        for c in range(self.num_classes):
            ...
            if len(class_features) == 0:
                raise ValueError(f"No samples for class {c} in few-shot set")

            proto = class_features.mean(0)
            prototypes.append(proto)

        self.prototypes = torch.stack(prototypes)  # store here

    def predict(self, x):
        assert self.prototypes is not None, "Prototypes not computed!"
