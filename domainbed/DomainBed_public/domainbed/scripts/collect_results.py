...
def save_grouped_records(input_dir, hparams_dir): # to be removed
    from pathlib import Path
    import json
    # output dir definition to keep input_dir tree

    records = []
    for i, subdir in tqdm.tqdm(list(enumerate(os.listdir(input_dir))),
                               ncols=80,
                               leave=False):
        results_path = os.path.join(input_dir, subdir, "results.jsonl")
        print(results_path)
        try:
    ...
    tree = input_dir.split("/")[-2:]
    tree = "/".join(tree)

    hparams_dir = hparams_dir + "/" + tree
    hparams_dir = Path(hparams_dir)

    hparams_dir.mkdir(parents=True, exist_ok=True)

    file_path = hparams_dir / "records.jsonl"
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--hparams_dir", type=str, required=True) # To be removed
    ...
    # save_grouped_records(input_dir=args.input_dir, hparams_dir=args.hparams_dir) # to be removed
