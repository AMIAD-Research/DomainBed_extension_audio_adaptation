...
def generate_noisy_dataset(
    ...
    train_json_path = DATA_DIR / "noisy_cwws_train_dataset_info.json"
    with open(train_json_path, "r") as f:
        train_info = json.load(f)

    used_train_noise = {
        entry["noise"]
        for entry in train_info["data"]
        if entry["noise"] is not None
    }
    ...
    noise_files = [
        ...
        for f in files
        if f.endswith(".wav")
        and get_subpath_from_folder(os.path.join(root, f), "wham_noise")
        not in used_train_noise
        ]
...
def get_data_from_CV(data_path, split):
    ...
    for l in LANGUAGES: 
        ...
        # ).drop_duplicates(subset="spk_id", keep="first") # select only one sample per speaker identity
...
def filter_used_data(train_root, speech_root, output_root, data_root):
    """
    Remove .mp3 files from speech_root and output_root if their speaker
    (client_id) is already present in train_root.
    """
    ...
    # ---------------------------
    # Step 1: Collect train files
    # ---------------------------
    train_files = set()
    for root, _, files in os.walk(train_root):
        for f in files:
            if f.endswith(".mp3"):
                train_files.add(os.path.basename(f))

    print(f"Found {len(train_files)} unique .mp3 files in train_root")
    ...
    for lang in LANGUAGES:
        ...
        print(f"\nLoading language: {lang}")

        for file in os.listdir(lang_dir):
            if file not in VALID_TSV:
                continue

            tsv_path = os.path.join(lang_dir, file)
            print(f"  → {file}")
            ...
            with open(tsv_path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    path_to_speaker[filename] = client_id

    print(f"\nTotal metadata entries loaded: {len(path_to_speaker)}")

    # ---------------------------
    # Step 3: Get speakers in train
    # ---------------------------
    train_speakers = set()
    missing = 0

    for f in train_files:
        if f in path_to_speaker:
            train_speakers.add(path_to_speaker[f])
        else:
            missing += 1

    print(f"Unique train speakers: {len(train_speakers)}")
    print(f"Missing entries: {missing}")

    # ---------------------------
    # Step 4: Remove files with same speakers
    # ---------------------------
    def clean_directory(target_root):
        removed = 0
        checked = 0
        skipped_no_meta = 0
        ...
        for root, _, files in os.walk(target_root):
            for f in files:
                if f.endswith(".mp3"):
                    ...
                    if filename not in path_to_speaker:
                        skipped_no_meta += 1
                        continue

                    speaker = path_to_speaker[filename]
        ...
        print(f"{target_root}: checked={checked}, removed={removed}, no_meta={skipped_no_meta}")

    # ---------------------------
    # Step 5: Apply cleaning
    # ---------------------------
    clean_directory(speech_root)
...
if __name__ == "__main__":
    ...
    clean_dataset_name = "Y_NEW_CWWS_TESTSET"
    noisy_dataset_name = f"Y_NOISY_{clean_dataset_name}"
    ...
    train_root = Path(path_to_data / "NOISY_AUDIO_DA_CWWS")
    
    parser.add_argument("--train_root",type=str, default=train_root, help="Directory containing data used for training")
    ...
    # parser.add_argument("--train_root",type=str, required=True, help="Directory containing data used for training")
    ...
    print(f"Filter samples used in CWWS trainset... ")
    filter_used_data(args.train_root, args.speech_root, args.output_root, args.commonvoice_root)
