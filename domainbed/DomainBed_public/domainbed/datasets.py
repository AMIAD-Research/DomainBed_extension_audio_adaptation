...
DATASETS = [
    ...
    "SpawriousM2M_hard",
    # Audio dataset
    "NOISY_AUDIO_DA", # to be removed
    "NOISY_AUDIO_DA_CWWS",
    "NOISY_AUDIO_DA_CWWS_6000",
    "NOISY_CWWS_TESTSET",
    "NOISY_CWWS_TESTSET_wind",
    "NOISY_AUDIO_DA_CWWS_mono",
    "NOISY_CWWS_TESTSET_mono"
    "Voxpopuli_verif",
    # OOP
    "VLCS_OOP",
    "PACS_OOP",
...
class MultipleDomainDataset:
    N_STEPS = 5001 #800           # Default, subclasses may override
    CHECKPOINT_FREQ = 100 #50    # Default, subclasses may override in the dataset class !
    # N_WORKERS = 8            # Default, subclasses may override
    N_WORKERS = 2            # Change the number of max workers to be able to run
...
class VLCS_OOP(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["C", "L", "S", "V"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "VLCS_OOP/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
class PACS_OOP(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "S"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "PACS_OOP/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
class DomainNet_OOP(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 1000
    ENVIRONMENTS = ["clip", "info", "paint", "quick", "real", "sketch"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "domain_net_OOP/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
class OfficeHome_OOP(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["A", "C", "P", "R"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "office_home_OOP/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
class TerraIncognita_OOP(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["L100", "L38", "L43", "L46"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "terra_incognita_OOP/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
from domainbed.scripts_audio.audio_classes import *
...
def default_audio_loader(path: str):
    """Load an mp3 file and return a waveform tensor"""
    waveform, sample_rate = torchaudio.load(path)  # [channels, time] 
    
    # Convert to PyTorch tensor and transpose to [channels, time]
    ...
    if waveform.shape[0] > 1:
        raise ValueError(
        f"Expected mono audio with shape (1, N), "
...
class AudioFolderMP3(DatasetFolder):
    """
    A generic data loader for audio files arranged like this:

        root/class_x/xxx.mp3
        root/class_x/xxy.mp3
        root/class_y/123.mp3
        root/class_y/nsdf3.mp3

    Args:
        root (str or Path): Root directory path
        transform (callable, optional): A function/transform that takes in a waveform
            and returns a transformed version
        target_transform (callable, optional): A function/transform applied to the target
        loader (callable, optional): Function to load audio given its path (default: torchaudio)
        is_valid_file (callable, optional): Function to check if a file is valid
        allow_empty (bool, optional): If True, allows empty folders (default: False)
    """
    ...
    def __init__(
        ...
        loader: Callable[[str], Any] = default_audio_loader,
        ...
        super().__init__(
            root,
            loader,
            AUDIO_EXTENSIONS if is_valid_file is None else None,
            transform=transform,
            target_transform=target_transform,
            is_valid_file=is_valid_file,
            allow_empty=allow_empty,
        )
        self.audio_files = self.samples  # list of (path, class_index)


class MultipleEnvironmentAudioFolder(MultipleDomainDataset):
    def __init__(self, root, test_envs, augment, hparams):
        super().__init__()
        if root is None:
            raise ValueError('Data directory not specified!')
        ...
        duration = 3  # seconds
        channels = 1  # mono
        target_length = duration * target_sample_rate  # nb second clips

        transform = transforms.Compose([
            Resample(target_sample_rate),
            RandomCropPad(target_length),
            Normalize(),
            DropSampleRate()
        ])
        ...
        augment_transform = transforms.Compose([
            Resample(target_sample_rate),
            RandomCropPad(target_length),
            RandomTimeShift(max_shift_pct=0.1),
            RandomGain(0.8, 1.2),
            AddNoise(0.005),
            Normalize(),
            DropSampleRate()
        ...
        for i, environment in enumerate(environments):
            ...
            if augment and (i not in test_envs): # /!\ Always set True in hparams_registry.py.
                env_transform = augment_transform
            else:
                env_transform = transform

            path = os.path.join(root, environment)
            env_dataset = AudioFolderMP3(path, 
                transform=env_transform)
            
            self.datasets.append(env_dataset)

        self.input_shape = (channels, target_sample_rate * duration)
        self.num_classes = len(self.datasets[-1].classes)

class AUDIO_01(MultipleEnvironmentAudioFolder):
    CHECKPOINT_FREQ = 500
    ENVIRONMENTS = ["clean", "wham", "wind"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "AUDIO_01/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


# class debug_audio_01(MultipleEnvironmentAudioFolder):
#     N_STEPS = 10
#     CHECKPOINT_FREQ = 1
#     ENVIRONMENTS = ["clean", "wham", "wind"]
#     def __init__(self, root, test_envs, hparams):
#         self.dir = os.path.join(root, "debug_audio_01/")
#         super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


# class NOISY_AUDIO_01_WHAM_NEAR_CLEAR_TRI(MultipleEnvironmentAudioFolder):
#     CHECKPOINT_FREQ = 500
#     ENVIRONMENTS = ["clean", "wham", "wind"]
#     def __init__(self, root, test_envs, hparams):
#         self.dir = os.path.join(root, "NOISY_AUDIO_01_WHAM_NEAR_CLEAR_TRI/")
#         super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)



# class NOISY_AUDIO_01_WHAM_REAL_TRI(MultipleEnvironmentAudioFolder):
#     CHECKPOINT_FREQ = 500
#     ENVIRONMENTS = ["clean", "wham", "wind"]
#     def __init__(self, root, test_envs, hparams):
#         self.dir = os.path.join(root, "NOISY_AUDIO_01_WHAM_REAL_TRI/")
#         super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


# class NOISY_AUDIO_01_WHAM_VERY_NOISY_TRI(MultipleEnvironmentAudioFolder):
#     CHECKPOINT_FREQ = 500
#     ENVIRONMENTS = ["clean", "wham", "wind"]
#     def __init__(self, root, test_envs, hparams):
#         self.dir = os.path.join(root, "NOISY_AUDIO_01_WHAM_VERY_NOISY_TRI/")
#         super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


class AUDIO_DA(MultipleEnvironmentAudioFolder):
    CHECKPOINT_FREQ = 500
    ENVIRONMENTS = ["clean", "wham", "wind"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "AUDIO_DA/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class NOISY_AUDIO_DA(MultipleEnvironmentAudioFolder):
    CHECKPOINT_FREQ = 500
    ENVIRONMENTS = ["clean", "wham", "wind"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_AUDIO_DA/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


class AUDIO_DA_CWWS(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "AUDIO_DA_CWWS/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class NOISY_AUDIO_DA_CWWS(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_AUDIO_DA_CWWS/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class NOISY_AUDIO_DA_CWWS_mono(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_AUDIO_DA_CWWS_mono/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


class NOISY_AUDIO_DA_CWWS_6000(MultipleEnvironmentAudioFolder):
    N_STEPS = 3000 
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_AUDIO_DA_CWWS_6000/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


class NOISY_CWWS_TESTSET(MultipleEnvironmentAudioFolder):
    N_STEPS = 3000 
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_CWWS_TESTSET/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class NOISY_CWWS_TESTSET_wind(MultipleEnvironmentAudioFolder):
    N_STEPS = 3000 
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_CWWS_TESTSET_wind/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class NOISY_CWWS_TESTSET_mono(MultipleEnvironmentAudioFolder):
    N_STEPS = 3000 
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_CWWS_TESTSET_mono/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
        

class Voxpopuli_verif(MultipleEnvironmentAudioFolder):
    N_STEPS = 3000 
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "Voxpopuli_verif/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
        

class NOISY_Voxpopuli_verif(MultipleEnvironmentAudioFolder):
    N_STEPS = 1500 
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "NOISY_Voxpopuli_verif/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
#####
class noisy_CWWS_train_small(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "noisy_CWWS_train_small/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


class noisy_CWWS_train_mono(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "noisy_CWWS_train_mono/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class noisy_CWWS_test(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "noisy_CWWS_test/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)


class noisy_CWWS_validation(MultipleEnvironmentAudioFolder):
    N_STEPS = 2000 
    CHECKPOINT_FREQ = 200
    ENVIRONMENTS = ["clean", "wham", "wind", "saturation"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "noisy_CWWS_validation/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)