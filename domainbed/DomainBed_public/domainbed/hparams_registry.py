...
def _hparams(algorithm, dataset, random_seed):
    ...
    _hparam('wav2vec2_base_100k_voxpopuli', True, lambda r: True) # Audio featurizer network
    ...
    _hparam('freeze_ln', False, lambda r: False) # Wav2Vec2 freeze Layer Normalization
    _hparam('freeze_all', True, lambda r: False) # Wav2Vec2 freeze all
    ...
    _hparam('wav2vec2_dropout', 0., lambda r: r.choice([0., 0.1, 0.5])) #Wav2Vec2 dropout
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('dann_adapt_coeff_disc', 1., lambda r: 10**r.uniform(-1, 1))
        _hparam('dann_adapt_coeff_gen', 1., lambda r: 10**r.uniform(-1, 1))
        ...
        _hparam('mmd_gamma', 1., lambda r: 10**r.uniform(-1, 1))
        _hparam('mmd_adapt_coeff_obj', 1., lambda r: 10**r.uniform(-1, 1))
        _hparam('mmd_adapt_coeff_pen', 1., lambda r: 10**r.uniform(-1, 1))
        ...
        _hparam('erm_adapt_coeff', 1., lambda r: 10**r.uniform(-1, 1))
    ...
    # if dataset == "AUDIO_01": 
    # if "AUDIO" in dataset:
    #     _hparam('lr', 5e-4, lambda r: 10**r.uniform(-4, -3)) #Audio learning rate def
    ...
    if dataset in SMALL_IMAGES:
        ...
        # print(f"change batch size when workin on A100")
        _hparam('batch_size', 64, lambda r: int(2**r.uniform(3, 5.5))) #audio batch_size def
        # _hparam('batch_size', 128, lambda r: int(2**r.uniform(3, 5.5))) #audio batch_size def
    else:        
        _hparam('batch_size', 2, lambda r: int(2**r.uniform(3, 5.5)))
